...
# Coordinate ranges
DISTANCE_MIN, DISTANCE_MAX = -150, 150
...
def make_callback(topic):
    ...
    def callback(msg):
        ...
        angle_raw = msg.point.y
        front_flag = msg.point.z
        ...
        if distance != 0.1163:
            if int(front_flag) == 0:
                lat = distance * math.cos(angle_raw)
                long = -1 * distance * math.sin(angle_raw)
            else:
                lat = distance * math.cos(angle_raw)
                long = distance * math.sin(angle_raw)
...
# Convert meters → screen coords relative to (0,0) = back bumper midpoint
def to_screen_coords(lx, ly, width, height, center_x, center_y):
    gx = int(center_x + (lx / (DISTANCE_MAX - DISTANCE_MIN)) * (width - 3))
    gy = int(center_y - (ly / (DISTANCE_MAX - DISTANCE_MIN)) * (height - 3))
    return gx, gy
...
def curses_display(stdscr):
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            for lx in frange(-half_width, half_width, step):
                for ly in frange(0, car_length, step):
                    x, y = to_screen_coords(lx, ly, width, height, center_x, center_y)
                    try:
                        stdscr.addch(y, x, 'X')
                    except curses.error:
                        pass
        ...
        with lock:
            for label, (lat, long) in track_points.items():
                ...
                # Avoid overlapping the centerline or the car
